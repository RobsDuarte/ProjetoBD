...
--
-- Host: 127.0.0.1
-- Tempo de geração: 16-Nov-2020 às 03:32
...
--
-- Estrutura stand-in para vista `altasazi`
-- (Veja abaixo para a view atual)
--
CREATE TABLE `altasazi` (
`pacid` int(10)
);
...
--
-- Estrutura stand-in para vista `altascloro`
-- (Veja abaixo para a view atual)
--
CREATE TABLE `altascloro` (
`pacid` int(10)
);
...
--
-- Estrutura stand-in para vista `altaslop`
-- (Veja abaixo para a view atual)
--
CREATE TABLE `altaslop` (
`pacid` int(10)
);
...
--
-- Estrutura para vista `altasazi`
--
DROP TABLE IF EXISTS `altasazi`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `altasazi`  AS  select `m`.`participantID` AS `pacid` from ((select `pacidazitrom`.`participantID` AS `participantID` from `pacidazitrom`) `m` join (select `vivos2`.`participantID` AS `participantID` from `vivos2`) `o` on(`m`.`participantID` = `o`.`participantID`)) ;

-- --------------------------------------------------------

--
-- Estrutura para vista `altascloro`
--
DROP TABLE IF EXISTS `altascloro`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `altascloro`  AS  select `m`.`participantID` AS `pacid` from ((select `pacidcloro2`.`participantID` AS `participantID` from `pacidcloro2`) `m` join (select `vivos2`.`participantID` AS `participantID` from `vivos2`) `o` on(`m`.`participantID` = `o`.`participantID`)) ;
...
--
-- Estrutura para vista `altaslop`
--
DROP TABLE IF EXISTS `altaslop`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `altaslop`  AS  select `m`.`participantID` AS `pacid` from ((select `pacidlop2`.`participantID` AS `participantID` from `pacidlop2`) `m` join (select `vivos2`.`participantID` AS `participantID` from `vivos2`) `o` on(`m`.`participantID` = `o`.`participantID`)) ;
